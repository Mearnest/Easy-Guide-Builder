...
if(!function_exists('strcode2utf')){ 
  function strcode2utf($str,$lo=true) {
	...
	// e flag is depracted as of PHP 5.5 in favor preg_replace_callback
	// $str = preg_replace('/\&\#([0-9]+)\;/me', "code2utf('\\1',{$lo})",$str);
	// $str = preg_replace('/\&\#x([0-9a-fA-F]+)\;/me', "codeHex2utf('\\1',{$lo})",$str);
	$func = 'return code2utf($matches[1],'. $lo .');';
	$str = preg_replace_callback('/\&\#([0-9]+)\;/m', create_function ('$matches', $func), $str);
	$func = 'return codeHex2utf($matches[1],'. $lo .');';
	$str = preg_replace_callback('/\&\#x([0-9a-fA-F]+)\;/m', create_function ('$matches', $func), $str);
	return $str;
  }
}
